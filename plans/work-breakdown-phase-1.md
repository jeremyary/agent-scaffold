...
**Stories:** 29 (across 8 features: F1, F2, F14, F18, F20, F21, F22, F25)
**Work Units:** 11 (WU-0 through WU-9, WU-8 split into WU-8a + WU-8b)
...
| Frontend | `work-breakdown-phase-1-chunk-ui.md` | WU-8a (Frontend Scaffold + Auth + Route Guards), WU-8b (Public Landing Page + Calculator) |
...
| **Frontend** | WU-8a -> WU-8b (sequential) | WU-8a: none after WU-0; WU-8b: WU-8a |

Convergence points:
- **WU-4** requires both WU-1 and WU-2 (merges data + auth streams)
- **WU-7** requires WU-4 and WU-3 (merges RBAC + HMDA)
- **WU-9** requires all WU-0 through WU-8b
...
| WU-5 | S-1-F18-01, S-1-F18-03, S-1-F21-01, S-1-F21-04 | F18, F21 | 4 |
| WU-6 | S-1-F20-01, S-1-F20-02, S-1-F20-03 | F20 | 3 |
| WU-7 | S-1-F14-01 to S-1-F14-05 (integration), S-1-F25-03 | F14, F25 | 6 |
| WU-8a | S-1-F2-02 (route guards), S-1-F20-05 (empty states) | F2, F20 | 2 |
| WU-8b | S-1-F1-01, S-1-F1-02, S-1-F1-03 | F1 | 3 |
| WU-9 | S-1-F22-01, S-1-F22-02, S-1-F22-03, S-1-F22-04 | F22 | 4 |
| **Total** | | | **35 story-tasks** (29 unique stories, some tested across multiple WUs) |
...
| WU-5 | LangFuse + Model Routing | 4 | S | Config loading, callback factory, model routing. Mostly wiring. 3 verification-only stories merged into implementation stories. |
...
| WU-8a | Frontend Scaffold + Auth | 12 | M | React scaffold, Keycloak auth, route guards, empty states. Foundational UI wiring. |
| WU-8b | Landing Page + Calculator | 11 | M | Hero, product cards, affordability calculator, chat stub, shadcn/ui. 4 files are generated shadcn components. |
...
### Auth Context Flow
WU-2 produces `get_current_user` dependency -> WU-4 wraps it with RBAC -> WU-7 tests the full pipeline -> WU-8a consumes via API client. Each WU must use the exact `UserContext` model from the TD hub.
...
WU-8a creates the shared TypeScript interfaces (`AuthUser`, `UserRole`, `HealthResponse`, `ErrorResponse`, `ProductInfo`, `AffordabilityRequest`/`Response`) and API client, which WU-8b consumes. Neither depends on running backend services -- the API client is implemented against type contracts from the TD hub. WU-9 verifies the full frontend-backend integration.
...
- **Frontend (WU-8a, WU-8b):** `plans/work-breakdown-phase-1-chunk-ui.md`
...
| TD-I-05 | Empty states for non-existent UIs | WU-8a covers landing page and Phase 1 route stub empty states only |
