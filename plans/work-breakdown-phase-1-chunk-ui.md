...
**Work Units:** WU-8a (Frontend Scaffold + Auth + Route Guards), WU-8b (Public Landing Page + Calculator)
**Stories:** S-1-F2-02, S-1-F20-05 (WU-8a); S-1-F1-01, S-1-F1-02, S-1-F1-03 (WU-8b)
**Features:** F1 (Prospect Landing Page and Affordability Calculator), F2 (Role-Based Route Access), F20 (Empty State Handling)
**Agent:** @frontend-developer
**Dependencies:** WU-8a depends on WU-0 (project bootstrap). WU-8b depends on WU-8a (scaffold, router, auth provider, shared types).

---

## Shared Context

This context applies to all stories in WU-8a and WU-8b. Read once before starting any story.
...
## WU-8a: Frontend Scaffold, Auth, and Route Guards

**Complexity:** M (Medium) -- 12 files: shared types/client, Keycloak auth, route guards, empty state, error page
**Agent:** @frontend-developer
**Dependencies:** WU-0 (project bootstrap)
**Stories:** S-1-F2-02 (route guards), S-1-F20-05 (empty states)

### Scope

WU-8a scaffolds the React SPA with TanStack Router, creates shared TypeScript types and API client, implements Keycloak OIDC integration, builds role-based route guards for all persona routes, and adds the reusable empty state component. After WU-8a completes, the app boots, authenticates, and routes to placeholder pages.

**File Budget:** 12 files:
- Shared infrastructure (2): types.ts, api-client.ts
- Auth (2): auth.ts, use-auth.ts
- Layout + routing (6): __root.tsx, borrower/index.tsx, loan-officer/index.tsx, underwriter/index.tsx, ceo/index.tsx, silent-check-sso.html
- Common components (2): error-page.tsx, empty-state.tsx
...
- `packages/ui/src/services/types.ts` -- Shared TypeScript interfaces (AuthUser, UserRole, HealthResponse, ErrorResponse, ProductInfo, AffordabilityRequest/Response)
- `packages/ui/src/services/api-client.ts` -- API client functions (fetchHealth, fetchProducts, calculateAffordability)
...
**Context files:**
- WU-8a/WU-8b Shared Context (above) -- Binding contracts, key decisions, route structure

**Requirements:**

Create the React app scaffold, shared types, API client, Keycloak OIDC authentication, and role-based route guards:

1. **Shared Types** (`types.ts`)
   - Create `packages/ui/src/services/types.ts` with all TypeScript interfaces from the Shared Context above: `UserRole`, `AuthUser`, `HealthResponse`, `ErrorResponse`, `ProductInfo`, `AffordabilityRequest`, `AffordabilityResponse`

2. **API Client** (`api-client.ts`)
   - Create `packages/ui/src/services/api-client.ts` with `fetchHealth()`, `fetchProducts()`, `calculateAffordability()` functions from the Shared Context above

3. **Keycloak Auth Service** (`auth.ts`)
...
4. **Auth Context Hook** (`use-auth.ts`)
...
7. **Error Page** (`error-page.tsx`)
   - Display 403 Forbidden or 404 Not Found message
   - Include "Go Home" button to return to landing page

8. **Silent SSO Check** (`public/silent-check-sso.html`)
   - Minimal HTML file that posts auth result back to parent frame
   - Standard Keycloak silent SSO pattern (see Keycloak docs)

**Steps:**

1. Install dependencies: `cd /home/jary/git/agent-scaffold/packages/ui && pnpm add keycloak-js @tanstack/react-router @tanstack/react-query`
2. Create `packages/ui/src/services/types.ts` with all TypeScript interfaces from the Shared Context
3. Create `packages/ui/src/services/api-client.ts` with API client functions from the Shared Context
4. Create `packages/ui/src/services/auth.ts` with Keycloak OIDC integration
5. Create `packages/ui/src/hooks/use-auth.ts` with auth context provider
6. Create `packages/ui/src/routes/__root.tsx` with root layout and auth provider
...
8. Create `packages/ui/src/components/common/error-page.tsx` for 403/404 handling
9. Create `packages/ui/public/silent-check-sso.html` for Keycloak silent SSO
10. Create header/footer stub components in `packages/ui/src/components/layout/`
11. Add AI compliance comment at top of all files: `// This project was developed with assistance from AI tools.`
12. Run TypeScript compiler: `pnpm exec tsc --noEmit` (verify no errors)
13. Write unit tests for auth service in `packages/ui/src/services/__tests__/auth.test.ts`
...
**Context files:**
- WU-8a/WU-8b Shared Context (above) -- Empty state design pattern
...
1. Create `packages/ui/src/components/common/empty-state.tsx` with reusable component
2. Add AI compliance comment at top of file: `// This project was developed with assistance from AI tools.`
3. Use `EmptyState` in all Phase 1 route stubs (borrower/LO/underwriter/CEO) -- coordinate with S-1-F2-02 implementation above
...
### WU-8a Exit Conditions

All stories in WU-8a must pass the following verification:
...
## WU-8b: Public Landing Page and Affordability Calculator

**Complexity:** M (Medium) -- 11 files but 4 are generated shadcn/ui components. 7 authored files.
**Agent:** @frontend-developer
**Dependencies:** WU-8a (scaffold, router, auth provider, shared types and API client)
**Stories:** S-1-F1-01, S-1-F1-02, S-1-F1-03

### Scope

WU-8b builds the public landing page with hero section, product cards, affordability calculator, and chat widget stub. It depends on WU-8a for the React scaffold, TanStack Router, shared types, and API client.

**File Budget:** 11 files:
- Route (1): routes/index.tsx
- Landing components (5): hero.tsx, product-cards.tsx, calculator.tsx, calculator-result.tsx, chat-widget-stub.tsx
- Validation (1): schemas/calculator.ts
- shadcn/ui components (4, generated): button.tsx, card.tsx, input.tsx, label.tsx
...
**Context files:**
- `packages/ui/src/services/types.ts` -- Read `ProductInfo`, `AffordabilityRequest`, `AffordabilityResponse` interfaces (created by WU-8a)
- `packages/ui/src/services/api-client.ts` -- Read `fetchProducts()` and `calculateAffordability()` functions (created by WU-8a)
- WU-8a/WU-8b Shared Context (above) -- Binding contracts and key decisions
...
1. Install dependencies: `cd /home/jary/git/agent-scaffold/packages/ui && pnpm add react-hook-form @hookform/resolvers zod`
...
3. Create Zod schema `packages/ui/src/schemas/calculator.ts`
4. Create hero component `packages/ui/src/components/landing/hero.tsx`
5. Create product cards component `packages/ui/src/components/landing/product-cards.tsx`
6. Create calculator form component `packages/ui/src/components/landing/calculator.tsx` (use react-hook-form + zod)
7. Create calculator result component `packages/ui/src/components/landing/calculator-result.tsx`
8. Create chat widget stub component `packages/ui/src/components/landing/chat-widget-stub.tsx`
9. Create landing page route `packages/ui/src/routes/index.tsx` that composes all components
10. Add AI compliance comment at top of all files: `// This project was developed with assistance from AI tools.`
11. Run TypeScript compiler: `pnpm exec tsc --noEmit` (verify no errors)
12. Write unit tests for calculator component in `packages/ui/src/components/landing/__tests__/calculator.test.tsx`

**Contracts:**

Use the following API client functions (created by WU-8a in `packages/ui/src/services/api-client.ts`):
...
Use these TypeScript interfaces (created by WU-8a in `packages/ui/src/services/types.ts`):
...
### WU-8b Exit Conditions

All stories in WU-8b must pass the following verification:
...
**TD-I-05:** Empty state handling references UIs that do not exist in Phase 1. Resolution: WU-8a implements empty states for landing page and Phase 1 route stubs only. Later phases add empty states for their respective UIs.

---

## AI Compliance

All files created in WU-8a and WU-8b must include the AI assistance comment at the top:
...
*WU-8 split into WU-8a + WU-8b during post-review fixes (Phase 12 triage).*
