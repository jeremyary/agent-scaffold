# PRD: AI Banking Quickstart -- Summit Cap Financial

## Problem Statement

Financial services organizations want to adopt agentic AI but face three interlocking challenges:

1. **No reference architecture** -- There is no publicly available, end-to-end example of agentic AI applied to a realistic financial services workflow that demonstrates role-based access control, regulatory compliance, multi-persona experiences, and model routing in a single cohesive application.

2. **Compliance fear** -- Regulated industries are hesitant to deploy AI because they cannot see how guardrails, audit trails, and regulatory awareness work in practice. Abstract architecture diagrams do not build confidence. The tension between collecting demographic data for regulatory reporting (HMDA) while simultaneously refusing to use it in lending decisions is one of the most compelling and misunderstood challenges in mortgage AI -- and no reference implementation demonstrates it.

3. **High barrier to experimentation** -- Developers and architects at financial institutions need a working, runnable application they can deploy locally or on enterprise infrastructure to evaluate agentic AI patterns against their own requirements. Nothing like this exists today for the mortgage lending domain.

**Who has this problem:** Red Hat customers and prospects in financial services who are evaluating agentic AI; Red Hat's own AI BU team who needs a compelling Summit demo; developers and architects who need a production-quality starting point.

**How they currently cope:** They build throwaway prototypes, watch conference talks without runnable code, or attempt to assemble patterns from disconnected tutorials and blog posts.

## Target Users

This application serves two distinct audiences with different needs:

### Audience 1: Demo Viewers (Red Hat Summit)

These are Red Hat Summit attendees watching a live Spotlight demo. They need to see a compelling, polished demonstration of agentic AI in financial services within a few minutes. They are not interacting with the application directly.

### Audience 2: Quickstart Users (Developers and Architects)

These are technical practitioners who download and run the Quickstart to evaluate agentic AI patterns. They need to understand the architecture, run the app locally, modify it, and use it as a foundation for their own applications.

### Application Personas

The application itself has five personas who interact with the system. These are the "characters" in the Quickstart:

### Persona: Prospect (Public Website Visitor)
- **Role:** Anonymous visitor to Summit Cap Financial's public website
- **Goals:** Understand mortgage products offered by Summit Cap Financial; determine if they might qualify for a mortgage; get answers to general mortgage questions; use affordability and payment estimation tools; explore pre-qualification
- **Pain Points:** Overwhelmed by mortgage jargon; unsure if they qualify; wants quick answers without calling a bank or filling out forms; confused about the difference between pre-qualification, pre-approval, and a full application
- **Context:** Browsing the Summit Cap Financial website from any device. Has not created an account or logged in. May be comparing multiple lenders. Interacts with a public-facing virtual assistant.

### Persona: Sarah Mitchell (Borrower / Customer)
- **Role:** Authenticated customer who is applying for or managing a mortgage loan with Summit Cap Financial
- **Goals:** Apply for a mortgage (starting with pre-qualification or going directly to full application); track application status including any conditions from underwriting; upload required documents; understand what is needed from her and when; ask questions about her specific loan terms, rate lock status, and timeline
- **Pain Points:** The mortgage process is opaque and stressful; does not know what stage her application is in or what conditions remain; has questions but does not want to wait on hold; unsure what documents are still needed or why previously submitted documents were rejected; confused when documents are sent back for correction
- **Context:** Logged into her Summit Cap Financial account. Has an active mortgage application (or is starting one). May be a sole borrower or part of a co-borrower pair. Interacts with a personal AI assistant that has access to her application data, documents, and loan details. Cannot see any other customer's data.

### Persona: James Torres (Loan Officer)
- **Role:** Summit Cap Financial employee who originates and manages mortgage applications. The LO role in this application absorbs some loan processor duties (document follow-up, condition clearing, file packaging) that in larger organizations would be handled by a separate processor role.
- **Goals:** Manage his pipeline of active applications; gather borrower information and documents; prepare applications for underwriting submission; clear conditions returned by underwriting; communicate with borrowers about status, missing documents, and conditions; track which applications need urgent attention based on rate lock expirations, closing dates, and stale files
- **Pain Points:** Juggling multiple applications at different stages; manually checking completeness of document packages; spending time on routine borrower questions that an AI could handle; conditions clearing is the most time-consuming post-submission activity; tracking rate lock expirations and approaching deadlines across many files
- **Context:** Logged into the employee portal. Sees only his own pipeline of applications (not other loan officers' work). Has an AI assistant that can help with application review, document completeness checks, conditions management, borrower communication drafts, and pipeline prioritization by urgency. Can recommend workflow transitions (e.g., "submit to underwriting") but must confirm them manually.

### Persona: Maria Chen (Underwriter)
- **Role:** Summit Cap Financial employee who evaluates mortgage applications for approval
- **Goals:** Review applications in her queue; assess creditworthiness and risk; verify compliance with lending regulations, agency/investor guidelines, and internal policies; make approval, conditional approval, suspension, or denial decisions; issue conditions and track their resolution; request additional documentation when needed; flag potential fraud indicators
- **Pain Points:** High volume of applications to review; must cross-reference multiple data sources (credit reports, property appraisals, income verification, regulatory requirements, investor guidelines); needs to document rationale for every decision for audit purposes; conditions loop back-and-forth is time-consuming
- **Context:** Logged into the employee portal. Can see all applications in the underwriting pipeline, including those still in progress with loan officers (read-only visibility into the full pipeline). Has an AI assistant with access to a compliance knowledge base (regulatory documents, agency/investor guidelines, and internal policies) that can help with risk assessment, flag potential compliance issues and fraud indicators, draft decision rationale, cite compensating factors, and answer regulatory questions. All AI recommendations are advisory -- the underwriter makes the final decision with human-in-the-loop confirmation.

### Persona: David Park (CEO)
- **Role:** Chief Executive Officer of Summit Cap Financial
- **Goals:** Monitor the health of the lending business; understand pipeline volume, pull-through rate, credit exposure, risk distribution, and loan officer performance; track turn times by stage; identify trends and potential problems before they become crises; review fair lending compliance through disparate impact metrics
- **Pain Points:** Data is scattered across systems; getting answers to business questions requires waiting for reports or asking analysts; wants real-time, conversational access to business metrics; needs to monitor fair lending compliance at the portfolio level
- **Context:** Logged into the executive portal. Sees aggregate dashboards showing pipeline health, pull-through rate, turn times by stage, denial rate and reasons, product mix distribution, loan officer workload, and fair lending metrics. Has an AI assistant that can answer drill-down questions about the data ("How many loans does James have in underwriting?", "What is our pull-through rate this quarter?", "Show me approval rates broken down by demographic segment"). Can drill down to individual deal details when needed, with partial PII visibility -- borrower names are visible for operational context, but sensitive fields (SSN, full account numbers, date of birth) are masked. Cross-session memory allows continuity ("Last time we discussed the Q1 pipeline -- has the situation improved?").

### Persona Gap: Loan Processor (Explicitly Out of Scope)

In production mortgage operations, the Loan Processor is a distinct role that sits between the Loan Officer and Underwriter, handling operational tasks: ordering title and appraisal, chasing conditions, verifying employment, and packaging files. This application absorbs processor duties into the Loan Officer role to keep the persona count at five. This is a deliberate simplification for the PoC -- a realistic expansion path for Quickstart users would be to split the LO persona into LO + Processor. The "Build Your Own Persona" extensibility pattern (see Downstream Notes) should use this as the example case.

## Proposed Solution

Build a full-stack web application branded as "Summit Cap Financial" that demonstrates agentic AI applied to the mortgage lending lifecycle. Summit Cap Financial is a fictional mortgage lender headquartered in Denver, Colorado, licensed to originate loans in Colorado, operating under both federal regulations and Colorado Division of Real Estate oversight. The application covers the process from initial prospect inquiry through pre-qualification, application, underwriting (including iterative conditions clearing), and approval.
...
| Quickstart documentation completeness | No docs | README with setup, architecture overview, deployment guide, and honest documentation of what is real vs. simulated | Developer can deploy to a container platform using only the provided docs | Quickstart |
| AI observability | Not observable | All agent interactions visible in a self-hosted observability dashboard | Trace a single request from user input through agent execution to response | Quickstart |
| Document processing | No capability | System can accept document uploads, extract relevant information, flag quality issues, and track document freshness | Upload a sample pay stub and verify extracted data appears in application; upload a blurry document and verify quality flag | Both |
| Cross-session memory | No memory | Conversations persist across browser sessions per user | Start a conversation, close the browser, return, and reference prior context | Demo |
